...
const ACCESS_TOKEN_LIFETIME  = 3 * SECONDS_PER_HOUR; // 3 hours
const REFRESH_TOKEN_LIFETIME = 6 * SECONDS_PER_HOUR; // 6 hours
    ...
    async handle (request, storage, options) {
        switch (request.body.grant_type) {
            ...
            case "refresh_token":
                ...
                if (old_token.token_type === "stored") {
                    options.access_token_lifetime  = 24*60*60; //  a day
                    options.refresh_token_lifetime = 90*24*60*60; // 90 days
                }
                const new_token = await this.generate_token(
                    null, storage, options
                );

                await storage.update_token({refresh_token}, new_token, options);
                new_token.token_type = old_token.token_type;
                return new_token;
